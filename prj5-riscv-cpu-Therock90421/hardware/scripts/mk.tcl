...
set riscv_dir sources/ip_catalog/riscv_core
set riscv_top_dir sources/hdl
...
if {$act == "rtl_chk" || $act == "sch_gen" || $act == "bhv_sim" || $act == "pst_sim" || $act == "bit_gen"} {
	...
	# RISC-V source files
	add_files -norecurse -fileset sources_1 ${script_dir}/../${riscv_dir}/

	# RISC-V top module and ideal memory
	add_files -norecurse -fileset sources_1 ${script_dir}/../${riscv_top_dir}/riscv_cpu_top.v
	...
	if {$act == "rtl_chk" || $act == "sch_gen" } {
		if {$act == "sch_gen"} {
			...
			set top_module riscv_cpu_top
		}

	} else {
		if {$act == "bhv_sim" || $act == "pst_sim" } {
			add_files -fileset constrs_1 -norecurse ${script_dir}/../constraints/riscv_cpu_simu.xdc
			...
			if {${bench_suite} == "hello"} {
				set_property verilog_define { {RISCV_CPU_FULL_SIMU} {UART_SIM} } [get_filesets sources_1]
				set_property verilog_define { {RISCV_CPU_FULL_SIMU} {UART_SIM} } [get_filesets sim_1]
			} else {
				set_property verilog_define { {RISCV_CPU_FULL_SIMU} } [get_filesets sources_1]
				set_property verilog_define { {RISCV_CPU_FULL_SIMU} } [get_filesets sim_1]
			}
			...
			source ${script_dir}/riscv_bram.tcl

			# set verilog simulator 
			set_property target_simulator "XSim" [current_project]

			# add testbed file and set top module to sim_1
			add_files -norecurse -fileset sim_1 ${script_dir}/../${tb_dir}/riscv_cpu_test.v
			add_files -norecurse -fileset sim_1 ${script_dir}/../${tb_dir}/uart_recv_sim.v

			set_property "top" riscv_cpu_test [get_filesets sim_1]
			update_compile_order -fileset [get_filesets sim_1]

		} else {
			
			# create Board Design (BD) with ZynqMP
			add_files -norecurse -fileset sources_1 ${script_dir}/../${riscv_top_dir}/axi_lite_if.v
		...
		add_files -norecurse -fileset sources_1 ${script_dir}/../${riscv_top_dir}/riscv_cpu_fpga.v
		set top_module riscv_cpu_fpga
	}
	...
	if {$act == "rtl_chk" || $act == "sch_gen"} {
		...
		# synthesizing design
		synth_design -top riscv_cpu_fpga -part ${device} -flatten_hierarchy none
